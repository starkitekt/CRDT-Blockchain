  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { error: 'txnId and 6-digit OTP are required' },
        { status: 400 }
      );
    }
    ...
    // ── Save verified KYC data to user ────────────────────────────────────────
    await connectDB();
    ...
    user.aadhaarSuffix     = kyc.aadhaarSuffix;  // ONLY last 4 digits
    ...
    auditLog({
      ...
      metadata:    { aadhaarSuffix: kyc.aadhaarSuffix },
    }).catch((err) => console.error('[auditLog] aadhaar verify:', err));

    return NextResponse.json({
      success:       true,
      message:       'Aadhaar KYC verified successfully',
      aadhaarSuffix: kyc.aadhaarSuffix,
    });
