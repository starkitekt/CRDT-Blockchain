...
export default function LoginPortal() {
  ...
  const handleSignup = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) { setAuthError(data.error ?? 'Registration failed. Please try again.'); return; }
            ...
            {mode === 'signup' && (
              <form onSubmit={handleSignup}>
                <Stack gap={5}>
                  ...
                  <InlineNotification
                    kind="info"
                    title="Profile details collected after sign-up."
                    subtitle="You'll complete your full profile in the next step after creating your account."
                    hideCloseButton
                    lowContrast
                  />
